...
raw_data_df = spark.read.format("delta").load("/mnt/signet-prod-us-east-1-data/analytics/audience_xfer/multi-touch-attribution/test4/raw/attribution_data")

# COMMAND ----------

import pyspark.sql.functions as F
display(raw_data_df.filter(F.col("individual_identity_key") == 'BB0DBwIBAgMEBQAABAUEDAwPBwUz'))

# COMMAND ----------

schema = raw_data_df.schema

# COMMAND ----------

raw_data_df = raw_data_df.withColumn("date", to_timestamp(col("date"),"yyyy-MM-dd HH:mm:ss"))\
...
dbutils.fs.rm("/mnt/signet-prod-us-east-1-data/analytics/audience_xfer/multi-touch-attribution/test2/bronze",recurse=True)
...
# MAGIC %md
# MAGIC **Note:** For the purpose of this solution accelerator, we have set the trigger to `once=True`. In production, you may choose to keep Auto Loader running so that data is loaded continuously as new files arrive.
...
# MAGIC %sql
# MAGIC 
# MAGIC drop table if exists signet_prod_audience_xfer.bronze;
