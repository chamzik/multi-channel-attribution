...
# MAGIC %sql
# MAGIC select * from transition_matrix
...
for message in transition_probability_pandas_df.start_state.to_list():
  if message!="Start":
    transition_probability_subset_pandas_df = get_transition_probability_graph(removal_state=message)
    ...
    removal_effect_per_channel[message] = round(((total_conversion_probability-new_conversion_probability)/total_conversion_probability), 2)
...
for message in removal_effect_per_channel.keys():
  conversion_attribution[message] = round(removal_effect_per_channel[message] / sum(removal_effect_per_channel.values()), 2)

messages = list(conversion_attribution.keys())
conversions = list(conversion_attribution.values())

conversion_pandas_df= pd.DataFrame({'attribution_model': 
                                    ['markov_chain' for _ in range(len(messages))], 
                                    'message':messages, 
...
# MAGIC ON markov_chain_attribution_update.attribution_model = gold_attribution.attribution_model AND markov_chain_attribution_update.message = gold_attribution.message
...
sns.catplot(x='message',y='attribution_percent',hue='attribution_model',data=attribution_pd, kind='bar', aspect=2)
