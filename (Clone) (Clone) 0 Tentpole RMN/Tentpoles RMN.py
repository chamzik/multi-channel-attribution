...
retailer_list=['AMAZON','KROGER','TARGET','WALMART']
...
conv_geo_df=conv_geo_df.withColumn("retailer",F.expr("case\
  ...
  ELSE 'OTHERS' END"))
conv_geo_df=conv_geo_df.filter(conv_geo_df.retailer.isin(retailer_list)).select("brand_id","individual_identity_key","order_sk","retailer")
...
calc1=master1.groupBy('event_name','retailer','event_start_date','event_end_date','date_filter').agg(F.countDistinct('individual_identity_key').alias('total_shoppers'),(F.sum('purchased_product_total_usd')/F.countDistinct('order_sk')).alias('threshold1'))
# threshold1 --- event_avg_spend_per_order
calc1_1=master1.groupBy('event_name','retailer','event_start_date','event_end_date','date_filter').agg(F.sum('purchased_product_total_usd').alias('total_spend'),F.sum('purchased_product_total_cnt').alias('total_items'),F.countDistinct('order_sk').alias('total_orders'))
...
id_event_spend=master1.groupBy('event_name','retailer','event_start_date','event_end_date','date_filter','individual_identity_key').agg((F.sum('purchased_product_total_usd')/F.countDistinct('order_sk')).alias('event_avg_spend_per_order')).withColumn('event_flag',F.lit(1))
id_order_df=master1.select("individual_identity_key","order_date").distinct()
id_df=id_order_df.select("individual_identity_key")

master2=conv_custom_geo_df.join(id_order_df,['individual_identity_key','order_date'],'leftanti')

master3=master2.join(id_df,'individual_identity_key','inner')
master4=conv_custom_geo_df.join(id_df,'individual_identity_key','inner')

id_overall_spend1=master3.groupBy('individual_identity_key').agg((F.sum('purchased_product_total_usd')/F.countDistinct('order_sk')).alias('overall_excl_event_avg_spend_per_order')).withColumn('overall_excl_event_flag',F.lit(1))
id_overall_spend2=master4.groupBy('individual_identity_key').agg((F.sum('purchased_product_total_usd')/F.countDistinct('order_sk')).alias('overall_avg_spend_per_order')).withColumn('overall_flag',F.lit(1))
...
df1=id_overall_spend1.join(id_event_spend,'individual_identity_key','fullouter')
df2=id_overall_spend2.join(df1,'individual_identity_key','fullouter')
...
display(calc2.groupBy('event_name','retailer','event_start_date','event_end_date','date_filter').agg(
...
calc2.write.format('delta').option("header","true").mode("overwrite").saveAsTable(f"{client}_prod_public_works.lm_tentpoles_rmn_growth")
...
df=spark.read.table(f"{client}_prod_public_works.lm_tentpoles_rmn_growth")
aud=df.fillna(0)
...
display(aud.groupBy('event_name','retailer').agg(F.countDistinct('individual_identity_key')))

# COMMAND ----------

display(aud.groupBy('event_name','retailer','event_start_date','event_end_date','event_flag','overall_flag','overall_excl_event_flag').agg(F.countDistinct('individual_identity_key')))

# COMMAND ----------

display(aud.groupBy('event_name','retailer','event_start_date','event_end_date','date_filter').agg(
  F.expr('percentile(growth1_1, array(0.25))')[0].alias('25th'),
  F.expr('percentile(growth1_1, array(0.50))')[0].alias('50th'),
  F.expr('percentile(growth1_1, array(0.75))')[0].alias('75th'),
  F.expr('percentile(growth1_1, array(0.90))')[0].alias('90th'),
  F.expr('percentile(growth1_1, array(0.95))')[0].alias('95th'),
  F.expr('percentile(growth1_1, array(0.99))')[0].alias('99th')))

# COMMAND ----------

display(aud.groupBy('event_name','retailer','event_start_date','event_end_date','date_filter').agg(
  F.expr('percentile(growth2_1, array(0.25))')[0].alias('25th'),
  F.expr('percentile(growth2_1, array(0.50))')[0].alias('50th'),
  F.expr('percentile(growth2_1, array(0.75))')[0].alias('75th'),
  F.expr('percentile(growth2_1, array(0.90))')[0].alias('90th'),
  F.expr('percentile(growth2_1, array(0.95))')[0].alias('95th'),
  F.expr('percentile(growth2_1, array(0.99))')[0].alias('99th')))

# COMMAND ----------

aud=aud.filter(aud.overall_excl_event_flag!=0)
...
display(aud.filter(aud.growth1>=0.05).groupBy('event_name','retailer','event_start_date','event_end_date').agg(F.countDistinct('individual_identity_key').alias('Increased_spenders')).withColumn('threshold',F.lit(0.05)))

# COMMAND ----------

display(aud.filter(aud.growth1>=0.1).groupBy('event_name','retailer','event_start_date','event_end_date').agg(F.countDistinct('individual_identity_key').alias('Increased_spenders')).withColumn('threshold',F.lit(0.1)))

# COMMAND ----------

display(aud.filter(aud.growth1>=0.15).groupBy('event_name','retailer','event_start_date','event_end_date').agg(F.countDistinct('individual_identity_key').alias('Increased_spenders')).withColumn('threshold',F.lit(0.15)))
